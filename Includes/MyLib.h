...
//{ //Operators with vect_t as a complex number://

const vect_t operator * (const vect_t& vect1, const vect_t& vect2);
const vect_t operator * (const vect_t& vect, const double& a);
const vect_t operator / (const vect_t& vect, const double& a);
const vect_t operator + (const vect_t& vect1, const vect_t& vect2);
const vect_t operator - (const vect_t& vect1, const vect_t& vect2);
...
const vect_t operator * (const vect_t& vect1, const vect_t& vect2)
    {
    return vect_t (vect1.x * vect2.x - vect1.y * vect2.y, vect1.x * vect2.y + vect2.x * vect2.y);
    }

const vect_t operator * (const vect_t& vect, const double& a)
    {
    return vect_t (vect.x * a, vect.y * a);
    }
...
class rect_t
    ...
    char* Print ()
        ...
        sprintf (text, "%lg %lg %lg %lg", x1, y1, x2, y2);
...
const vect_t MaxWindowSize (1600, 1200);

vect_t GetWindowSize ();

const vect_t Window (GetWindowSize ());
...
bool     SetColors (double thinknessLines, COLORREF color);
...
bool     DeleteSymbols (char * text, int length, char symbol);
...
bool CreateMyWindow (vect_t window, bool cursor)
    ...
    TX_LIB txCreateWindow ((int) window.x, (int) window.y);
    DIRECT_TX DirectTXCreateWindow ((int) window.x, (int) window.y, "MyWindow");
...
vect_t GetWindowSize ()
    ...
    if (window.x > MaxWindowSize.x)
        window.x = MaxWindowSize.x;

    if (window.y > MaxWindowSize.y)
        window.y = MaxWindowSize.y;
...
bool SetColors (double thinknessLines, COLORREF color)
    {
    SetColors (thinknessLines, color, color);
...
bool DrawRect (rect_t rect)
    {
    DEBUG printf ("Нарисован прямоугольник с координатами %lg, %lg, %lg, %lg\n", rect.x1, rect.y1, rect.x2, rect.y2);
...
bool DrawEllipse (vect_t point1, vect_t point2, int thinkness)
    {
    ...
    if ((point2.x - point1.x > thinkness * 2) && (point2.y - point1.y > thinkness * 2))
        {
        SetColors ((double) thinkness, txGetColor (), txGetFillColor ());

        point1.x += thinkness / 2;
        point1.y += thinkness / 2;
        point2.x -= thinkness / 2;
        point2.y -= thinkness / 2;
        }
    else
        {
        SetColors (txGetColor ());
        }
...
bool AllGoneBad (char* text)
    ...
    if (printMessage) printf ((const char*)sorrying);
...
char* PrintColor (COLORREF color)
    ...
    sprintf (text, "%d %d %d", (int) (color      ) & 0xFF,
                               (int) (color >> 8 ) & 0xFF,
                               (int) (color >> 16) & 0xFF);
...
bool DeleteSymbols (char * text, int length, char symbol)
    ...
    for (int number = 0; number < length; number++)
        if (text [number] == symbol)
            {
            if (number < length - 1)
                for (int num = number; num < length - 1; num++)
                    text [num] = text [num + 1];
            else
                text [number] = ' ';
            }
